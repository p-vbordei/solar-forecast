...
def create_pvsystem_from_config(plant_config: Dict[str, Any]) -> PVSystem:
    ...
    try:
        ...
        if huawei_model is not None:
            # Scale inverter to match plant AC capacity
            total_ac_capacity_kw = inverter.get('ac_power_rating_kw', 15000)
            scale_factor = (total_ac_capacity_kw * 1000) / huawei_model['Paco']

            # Use exact PVLIB database parameters scaled up to plant size
            inverter_params = {
                'Paco': huawei_model['Paco'] * scale_factor,     # Scale AC power to plant capacity
                'Pdco': huawei_model['Pdco'] * scale_factor,     # Scale DC power proportionally
                'Vdco': huawei_model['Vdco'],                    # DC voltage (same)
                'Pso': huawei_model['Pso'] * scale_factor,       # Scale standby power
                'C0': huawei_model['C0'],         # Efficiency curve coefficients (same)
                'C1': huawei_model['C1'],
                'C2': huawei_model['C2'],
                'C3': huawei_model['C3'],
                'Pnt': huawei_model['Pnt'] * scale_factor,       # Scale tare loss
            }
            print(f"PVLIB HUAWEI scaled: {huawei_model['Paco']/1000:.1f}kW × {scale_factor:.1f} = {inverter_params['Paco']/1000:.0f}kW AC")
        else:
        ...
        # Use total AC capacity from config (already in kW)
        total_ac_capacity = inverter.get('ac_power_rating_kw', 15000) * 1000  # Convert kW to Watts
        ...
        print(f"Manual inverter sized to: {total_ac_capacity/1000:.0f}kW AC")
...
def prepare_weather_data(weather_data: pd.DataFrame) -> pd.DataFrame:
    ...
    # Add default precipitable_water if not present (required for spectral model)
    if 'precipitable_water' not in weather.columns:
        # Default value for Romania: 14mm (moderate humidity)
        weather['precipitable_water'] = 14.0

    # Add default pressure if not present
    if 'pressure' not in weather.columns:
        # Standard atmospheric pressure at sea level
        weather['pressure'] = 101325  # Pa
...
def run_forecast(location: Location,
    ...
    if model == 'chain':
        ...
        prepared_weather = prepare_weather_data(weather)
        ...
        mc = ModelChain(
            ...
            location,
            aoi_model='physical',
            spectral_model='no_loss',  # Changed from 'first_solar' which caused NaN
            temperature_model='sapm',
            losses_model='pvwatts'
        )

        # Run the model with prepared weather data
        mc.run_model(prepared_weather)
        ...
        # Ensure weather has a proper index
        if not hasattr(weather.index, 'to_pydatetime'):
        ...
        # Get total AC capacity from inverter parameters
        capacity_w = system.inverter_parameters['Paco']
        capacity_mw = capacity_w / 1_000_000
        ...
        night_mask = solar_elevation.values <= -6  # Use .values to avoid index issues
        ac_power.iloc[night_mask] = 0  # Allow production down to -6° for dawn/dusk
        dc_power.iloc[night_mask] = 0
    ...
    """
    # Convert from W to MW and kW for easier interpretation
    results['ac_power_mw'] = results['ac_power'] / 1000000.0
    results['dc_power_mw'] = results['dc_power'] / 1000000.0
    results['ac_power_kw'] = results['ac_power'] / 1000.0  # For shoulders enhancement
    results['dc_power_kw'] = results['dc_power'] / 1000.0  # For shoulders enhancement
    ...
    results['ac_power_kw'] = results['ac_power_kw'].clip(lower=0)
    results['dc_power_kw'] = results['dc_power_kw'].clip(lower=0)
