...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    const includeStatistics =
      url.searchParams.get("includeStatistics") === "true";
      ...
      return json(
        {
          success: false,
          error:
            "Missing required parameters. Provide location/locationIds, start, and end dates.",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      return json(
        {
          success: false,
          error: "Invalid location ID format. Use comma-separated strings.",
        },
        { status: 400 },
      );
    }
    ...
    if (response.success) {
      ...
      return json({
        ...
        metadata: {
          ...
          location:
            parsedLocationIds.length === 1
              ? parsedLocationIds[0].toString()
              : "multiple",
          interval,
    ...
    if (error instanceof Error) {
      if (error.message.includes("Invalid date range")) {
        errorMessage =
          "Invalid date range. Ensure start date is before end date and within 5-year limit.";
      } else if (error.message.includes("location ID")) {
        errorMessage =
          "Invalid location ID. Ensure location exists and you have access.";
      } else if (error.message.includes("Raw data queries")) {
        errorMessage =
          "Raw data requests are limited to 30 days maximum. Use aggregated data for longer periods.";
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const historicalRequest: HistoricalDataRequest = {
      ...
      dataQualityFilter: body.dataQualityFilter || [
        DataQualityFilter.EXCLUDE_POOR,
      ],
    ...
    const response =
      await historicalService.getHistoricalData(historicalRequest);
