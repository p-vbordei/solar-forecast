...
import { LocationNotFoundError, LocationExistsError } from '$lib/utils/ApiErrors';
...
export class LocationsService {
    ...
    private convertStatusToEnum(status: string): LocationStatus {
        ...
        if (!Object.values(LocationStatus).includes(statusUpper as LocationStatus)) {
            throw new Error(`Invalid status: ${status}. Valid values are: ${Object.values(LocationStatus).join(', ')}`);
        }
    ...
    private async findLocationByName(name: string) {
        ...
        // Get location from repository
        const location = await this.repository.findByName(whereClause, selectFields);
     ...
     */
    private transformToPrismaCreateData(locationData: CreateLocationRequest) {
        return {
            // Required fields
            clientId: 1, // TODO: Get from authenticated user context
    ...
    async createLocation(locationData: CreateLocationRequest) {
        ...
        const prismaData = this.transformToPrismaCreateData(locationData);
     ...
     */
    async updateLocation(locationId: string, updateData: Partial<CreateLocationRequest>) {
